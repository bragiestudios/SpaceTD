...
public class GooglePlayServicesExtension extends RunnerSocial 
{
	...
	private static final int GooglePlayServices_Our_Info = 1002; //Set to match runtime e_achievement_our_info 
	...
	public void Init()
	{
		...
		double status = GooglePlayServices_Status();
		...
		else { Log.i("yoyo", "!! Google Play Services not initialising, as Google Play Services Connection status returned " + status + " !!"); }
	}
	...
	public void onStart() 
	{
		...
		// Uncomment this line if you want auto sign-in
		// Login();
	}
	...
	{
		return (double)GoogleApiAvailability.getInstance().isGooglePlayServicesAvailable(RunnerJNILib.ms_context);
	}
	...
	public void Login()
	{
		if(isSignedIn())
		{
			onLoginSuccess(getCurrentAccountSignedIn()); //Do this here so that the async messages are sent again
	...
	private void onLoginFailed()
	{
		// Send social event
		int dsMapIndex = RunnerJNILib.jCreateDsMap(null, null, null);
		RunnerJNILib.DsMapAddString( dsMapIndex, "type", "login_failed" );
		RunnerJNILib.DsMapAddDouble( dsMapIndex, "id", GooglePlayServices_Our_Info);
		RunnerJNILib.DsMapAddString( dsMapIndex, "name", "Not logged in" );
